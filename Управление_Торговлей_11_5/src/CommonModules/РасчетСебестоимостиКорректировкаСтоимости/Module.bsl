...
Функция ТекстСуммыПрочихРасходов(ПараметрыРасчета) Экспорт
	...
	|	ДД.ВременнаяРазница КАК ВременнаяРазница,
	|	(ВЫБОР
	// Исключаем суммы НУ, сформированные при распределении дополнительных расходов по товарам, кроме статьи "НДС налогового агента",
	// т.к. по статье "НДС налогового агента" не может быть разных вариантов распределения в регл. и нал. учете.
	...
	|		КОГДА НЕ Распределение.Ссылка ЕСТЬ NULL
	|		 И Распределение.СтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.НДСНалоговогоАгента)
	|			ТОГДА 0
	|		ИНАЧЕ ДД.СуммаРегл
	|		- ДД.ПостояннаяРазница
	...
	|		И Распределение.НазначениеНастройкиРаспределения = ЗНАЧЕНИЕ(Перечисление.НазначениеПравилРаспределенияРасходов.РаспределениеРасходовНаСебестоимостьТоваров)
	|ГДЕ
	|	&ПартионныйУчетВерсии22
	|	И НЕ ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.РаспределениеДоходовПоНаправлениямДеятельности)
